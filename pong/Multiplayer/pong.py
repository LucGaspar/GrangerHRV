# Script original por Nikried Huber (https://gitlab.fh-ooe.at/makerspace/reinforcement-learing/-/blob/master/src/pong.py)
# Utilizado e editado com a anuência do autor.
# Utilizado e alterado por Lucas Gasparino.
...
PLAYER_MOVEMENT_UNITS = 7
COMPUTER_MOVEMENT_UNITS = 7
...
def computer_predict(ball, computer):
    ...
    for x in range(ball.x + ball.radius, computer.x, ball.speed):
        if y - ball.radius < 0 or y + ball.radius > SCREEN_HEIGHT:
            predicted_dir *= -1
        y += ball.speed * predicted_dir * (random.randint(5, 10) * 0.1)
    return y
...
def writeInitialTime():
    ntp_time = None
    client = NTPClient()

    for host in ['a.st1.ntp.br', 'b.st1.ntp.br', 'c.st1.ntp.br', 'd.st1.ntp.br']:
        try:
            response = client.request(host)
            ntp_time = dt.datetime.fromtimestamp(response.orig_time)
            break
        except (NTPException):
            pass

    lTime = dt.datetime.now().strftime("%a %b %d %H:%M:%S.%f")
    iTime = ntp_time.strftime("%a %b %d %H:%M:%S.%f")		
    
    ticks = pygame.time.get_ticks() 
    time = {'SystemTime': lTime, 'InternetTime': iTime, 'PyGameTicks': ticks}
    
    dfTime = pd.DataFrame(columns = ['SystemTime', 'InternetTime', 'PyGameTicks'])
    dfTime.append(time, ignore_index = True).to_csv('SyncTime.csv')
...
tick = 0
last_key = 1
df = pd.DataFrame(columns = ['TicksTime', 'BallPosX', 'BallPosY', 'BallSpeed', 'Player1X', 'Player1Y', 'Player2X', 'Player2Y', 'Score1', 'Score2'])
...
while True:
    ...
    display_text(str(player.score), 300, 45)
    display_text(str(computer.score), 1066, 45)
    ...
    if keys[pygame.K_s]:
        goal_reached, overlap = computer.collides_with_ball(ball)
        if goal_reached and computer.y + computer.height + COMPUTER_MOVEMENT_UNITS > ball.y - ball.radius:
            continue
        computer.y = SCREEN_HEIGHT - computer.height if computer.y + computer.height + COMPUTER_MOVEMENT_UNITS > SCREEN_HEIGHT else computer.y + COMPUTER_MOVEMENT_UNITS

    if keys[pygame.K_w]:
        goal_reached, overlap = computer.collides_with_ball(ball)
        if goal_reached and computer.y - COMPUTER_MOVEMENT_UNITS < ball.y + ball.radius:
            continue
        computer.y = 0 if computer.y - COMPUTER_MOVEMENT_UNITS < 0 else computer.y - COMPUTER_MOVEMENT_UNITS 
    ...
    recordData = {'TicksTime': pygame.time.get_ticks(),
    'BallPosX': ball.x,
    'BallPosY': ball.y,
    ...
    df = df.append(recordData, ignore_index = True)
